...
%}
    ...
    histrange = 1 : vocab_size;
    vocab = vocab';    
    M = size(image_paths,1);
    fprintf('Computing bag of sift feature.Number of images %d',M);
    image_feats = zeros(M,vocab_size);
    parfor i = 1 : M
        fprintf('Computing sift for %d \n',i);
        I = imread(image_paths{i});
      %  [l, f] = vl_dsift(single(I), 'fast','size',5);
        [l, f] = vl_dsift(single(I));
      % f = f';
        D = vl_alldist2(f,uint8(vocab'));
        numf = size(f,2);
        ...
        %end
        [mv , index] = min(D,[],2);
        hist = histc(index,histrange);
    %normalize    
    %range = max(hist) - min(hist);
    mx = max(hist);mn = min(hist);
    range = mx - mn;
    hist = (hist - mn) / range;    
    image_feats(i,:) = hist;
    end
