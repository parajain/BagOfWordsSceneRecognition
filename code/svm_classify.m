...
%training svm
% need d x N features, W will be dimension d size  d x 1
w = [];
LAMBDA = 0.001;
for class = 1 : num_categories
    binary_labels = strcmp(train_labels,categories(class));
    binary_labels = binary_labels - (~binary_labels);
   
    [w(:,class) b(class)] = vl_svmtrain(train_image_feats', double(binary_labels'),LAMBDA);
end

numoftest = size(test_image_feats,1);
test_prob = zeros(numoftest,num_categories);

for test = 1 : numoftest
    for cat = 1 : num_categories
        test_prob(test,cat) = w(:,cat)'*test_image_feats(test,:)' +  b(cat);
    end
end


[mx,In] = max(test_prob,[],2);
%predicted_categories = cell(numoftest,1);
predicted_categories = categories(In);
end
