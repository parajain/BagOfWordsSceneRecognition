...
% visual word vocabulary.
    ...
    M = size(image_paths,1);
     if ~exist('allfeatures.mat', 'file')
        fprintf('No existing features found. Computing one from training images\n')
        allfeat =[];
        for i = 1 : M
            I = imread(image_paths{i});
            binSize = 8 ;
           % [loc, f] = vl_dsift(single(I), 'size', binSize,'fast');
            [loc, f] = vl_dsift(single(I));
            f = f';
            nRows = size(f,2); % number of rows
            nSample = floor(nRows/10); % number of samples
            rndIDX = randperm(nRows); 
            newSample = f(rndIDX(1:nSample), :); 
            allfeat = vertcat(allfeat,newSample);
        end

        save('allfeatures.mat', 'allfeat')
     else
         load('allfeatures.mat')
     end
    [vocab, assignments] = vl_kmeans(single(allfeat'), vocab_size);
end
